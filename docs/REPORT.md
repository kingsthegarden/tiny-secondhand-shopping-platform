...
기능 우선으로 작성했다면 존재했을 약점 47건을 식별하고 모두 조치했다. 전체 목록과 코드 위치는 저장소의 **SECURITY.md** 참고. 대표 사례:
...
| 관리자 자기 잠금 | 관리자가 사용자 관리 화면에서 자기 자신의 role/status도 변경 가능 | 자기 자신 변경 자체를 차단(다른 관리자 계정 유도) |

## 5. 체크리스트 작성 및 테스팅

강의 제공 체크리스트를 기준으로 **CHECKLIST.md**를 작성하고, 각 항목을 pytest 자동화 테스트로 검증했다.

- 테스트 37개: 기능 테스트(가입→로그인→상품 등록→검색→상세, 프로필/비밀번호 변경, 관리자 기능) + 보안 테스트(SQLi, XSS, CSRF, IDOR, 잠금, 휴면, 계정 열거 타이밍, 회원가입 레이트리밋, 흔한 비밀번호 차단, 관리자 자기 잠금 방지, 위조 이미지 업로드, 음수 송금, 중복 신고, 임계값 자동 차단, 보안 헤더, HttpOnly 쿠키)
- 특징: **CSRF 보호를 끄지 않고** 실제 토큰을 파싱해 테스트 → 보안 기능이 켜진 실제 동작 그대로 검증
- 결과: `python -m pytest tests/ -v` → 37 passed

## 6. 유지보수

- 실사용 점검: 서버 구동 후 전 페이지 수동 확인, ngrok으로 외부 접속 시나리오 확인. 실제로 관리자 계정으로 로그인해 사용자 관리 화면을 확인하던 중 관리자 본인 계정도 목록에 노출되어 있고 role을 바꿀 수 있는 상태임을 발견 → 자기 자신의 role/status 변경을 서버·UI 양쪽에서 차단하도록 수정(SECURITY.md 5-5)
- 사용성 개선: 상품 상세에서 판매자와 바로 1:1 채팅/송금/신고로 이동하는 동선 추가, 프로필에서도 동일 동작 제공
- 상품 목록이 완전히 비어 있어 기능을 체감하기 어렵다는 점을 확인 → `flask seed-demo` CLI로 데모 상품 6개를 선택적으로 채워 넣을 수 있게 함(기본 실행 시에는 자동 생성되지 않음)
